...
import Queen from '@/components/game/Queen';
...
const LevelBoard = ({ id, level }: { id: string; level: Level }) => {
  ...
  const handleSquareClick = (row: number, col: number) => {
    ...
    const clashingPositions = getClashingQueens(newBoard, boardSize, colorRegions);
  ...
  const getQueenPositionForGivenX = (xRow: number, xCol: number, newBoard: string[][]) => {
    const directions = [
      [-1, 0],
      [1, 0],
      [0, -1],
  ...
  const placeQueen = (newBoard: string[][], row: number, col: number) => {
    newBoard[row][col] = 'Q'; // Place the queen

    console.log('autoPlaceXs', autoPlaceXs);
  ...
  const removeQueen = (newBoard: string[][], row: number, col: number) => {
    newBoard[row][col] = ' '; // Remove the queen
    ...
    setQueenGeneratedXs((prev) => {
      ...
      Object.keys(updated).forEach((xKey) => {
        if (updated[xKey].has(queenKey)) {
          ...
          if (!queenPos) {
            newBoard[xRow][xCol] = ' '; // Remove X if no queens depend on it
  ...
  return (
    <div key={id} className="flex flex-col items-center justify-center pt-4">
      <div className="flex flex-col items-center">
        <div>
          <div
            className={`flex w-full items-center space-x-4 py-1 sm:justify-between sm:space-x-0 ${
              showClock ? 'mb-0' : 'mb-2'
            }`}
          >
            <div className="flex items-center space-x-2">
              <h2 className="text-center text-xl">Level {id}</h2>
            </div>
            ...
            <div className="flex flex-1 justify-end sm:flex-none">
              <div className="relative flex items-center">
                {completed && (
                  <Queen
                    size="24"
                    className="absolute top-1/2 right-full mr-2 -translate-y-1/2 transform fill-yellow-400"
                  />
                )}
                <button
                  onClick={() => {
          ...
          <div className="flex justify-end">
            <Timer isGameWon={hasWon} onTimeUpdate={handleTimeUpdate} showTimer={showClock} />
          </div>
