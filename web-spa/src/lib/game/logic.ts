...
export const isSafeToPlaceQueen = (board: Board, row: number, col: number, size: number): boolean => {
...
export const getClashingQueens = (board: Board): Position[] => {
  ...
  Object.entries(queensPerRegion).forEach(([region, count]) => {
    if (count > 1) {
      for (let row = 0; row < size; row++) {
        for (let col = 0; col < size; col++) {
          if (board[row][col].color === region && board[row][col].value === 'Q') clashes.push({ row, col })
        }
      }
    }
  })
